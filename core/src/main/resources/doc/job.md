...
A **Trigger** determines when the job should run. It activates job execution e.g. based on a daily schedule or on
received events. The trigger creates an initial data item, which is passed to the job's first action.

Each data item is then handed over to the following action and so on. Think about the "Java Stream API" to get an idea
of the process.
...
Num Threads | The number of threads the job uses per action to process data items. Note that some actions may require execution in a single thread, which is then automatically handled by the job.
