# FTPS File Service
A service providing access to an FTPS server.

# Parameters
The FTPS service can be configured by the following parameters:
...
Passive mode | If checked, the service will use FTP passive mode to connect to the FTPS server.
Buffer size | The size of the buffer that is used by the service to e.g. copy files.
...
Windows ftp | If checked, the service expects the FTPS server to be running on a Microsoft Windows operating system.