...
The limit counter is set on job start and applies to the complete job execution.
Each processed data item increases the counter, which is never reset until the next job execution.
Since event triggered jobs don't stop their execution, the counter would never be reset.
