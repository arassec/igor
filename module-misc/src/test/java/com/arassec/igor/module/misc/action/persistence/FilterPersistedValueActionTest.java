...
class FilterPersistedValueActionTest extends MiscActionBaseTest {
    ...
    void testProcessNoValue() {
        ...
        verify(persistentValueRepositoryMock, times(0)).isPersisted(eq(JOB_ID), eq(TASK_ID), any(PersistentValue.class));
    }
    ...
    void testProcessNonPersistent() {
        ...
        verify(persistentValueRepositoryMock, times(1)).isPersisted(eq(JOB_ID), eq(TASK_ID), argCap.capture());
