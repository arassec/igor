  ...
  <core-panel class="action-editor">
    ...
    <button-row>
      <p slot="right">
        ...
        <input-button icon="arrow-up" v-on:clicked="$emit('move-up')"/>
        <input-button icon="arrow-down" v-on:clicked="$emit('move-down')"/>
      </p>
    </button-row>
...
<script>
  ...
  export default {
    ...
    props: ['action', 'actionIndex', 'taskIndex', 'testResults'],
    data: function () {
      return {
        actionTypes: []
      }
