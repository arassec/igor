    ...
    <div class="sticky max-width" :data-e2e="dataE2eName()">
        <core-panel class="min-height">
            <layout-row>
                <template v-slot:left>
                    <h1 class="truncate" data-e2e="title">
            ...
            <div class="table full-width">
                <div class="tr">
                    ...
                    <div class="td align-left">
                        <font-awesome-icon
                            :icon="action.active ? 'check-square' : 'square'"
                ...
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    ...
                    <div class="text-top td">
                        <textarea
                            ...
                            @change="changeActionDescription"
                            class="textarea"
                        />
                    </div>
                </div>
        ...
        <div>
            <core-panel class="spacer-top">
                <layout-row>
                    ...
                    <template v-slot:right>
                        <icon-button
                            icon="question"
                            v-show="hasDocumentation(action.type.key)"
                ...
                <div class="table margin-bottom">
                    <div class="tr" v-if="actionCategories.length > 0">
                        ...
                        <div class="td">
                            <select
                                :value="action.category.key"
                                @change="changeActionCategory($event)"
            ...
            <core-panel class="spacer-top">
                ...
                <p v-else>This Action has no parameters to configure.</p>
            </core-panel>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        loadCategories: async function () {
            await IgorBackend.getData("/api/category/action")
                .then((categories) => {
        ...
        loadTypesOfCategory: async function (categoryKey, selectFirst) {
            await IgorBackend.getData("/api/type/action/" + categoryKey)
                .then((types) => {
                ...
                .catch(() => {
                    console.error("Could not load action types for category: " + categoryKey);
                });
        },
        loadParametersOfType: async function (typeKey) {
            ...
            IgorBackend.getData("/api/parameters/action/" + typeKey)
                .then((parameters) => {
