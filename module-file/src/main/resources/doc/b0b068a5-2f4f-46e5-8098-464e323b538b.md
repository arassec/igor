# FTP File Service
A service providing access to an FTP server.

# Parameters
The FTP service can be configured by the following parameters:
...
Passive mode | If checked, the service will use FTP passive mode to connect to the FTP server.
...
Windows ftp | If checked, the service expects the FTP server to be running on a Microsoft Windows operating system.