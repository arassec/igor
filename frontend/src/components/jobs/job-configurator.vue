            ...
            <div class="table">
                <div class="tr">
                    ...
                    <div class="td align-left">
                        <font-awesome-icon
                            :icon="jobConfiguration.active ? 'check-square' : 'square'"
                ...
                <div class="tr">
                    ...
                    <div class="td">
                        <input-validated
                            id="name"
                            type="text"
                            @input="$emit('change-job-name', $event)"
                            :parent-id="jobConfiguration.id"
                            :property-id="'name'"
                            :validation-errors="validationErrors"
                            :model-value="jobConfiguration.name"
                ...
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    ...
                    <div class="td">
                        <textarea
                            ...
                            @input="$emit('change-job-description', $event.target.value)"
                            class="textarea"
                        />
                    </div>
                </div>
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    <div class="td">
                        <label for="numexechistory-input">History Limit</label>
                    </div>
                    <div class="td">
                        <input-validated
                            id="numexechistory-input"
                            type="text"
                            :parent-id="jobConfiguration.id"
                            :property-id="'historyLimit'"
                            :validation-errors="validationErrors"
                            :model-value="jobConfiguration.historyLimit"
                ...
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    <div class="td">
                        <label for="numsimlimit-input">Simulation Limit</label>
                    </div>
                    <div class="td">
                        <input-validated
                            id="numsimlimit-input"
                            type="text"
                            :parent-id="jobConfiguration.id"
                            :property-id="'simulationLimit'"
                            :validation-errors="validationErrors"
                            :model-value="jobConfiguration.simulationLimit"
                ...
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    <div class="td">
                        <label for="faulttolerant-input">Fault tolerant</label>
                    </div>
                    <div class="td align-left">
                        <font-awesome-icon
                            id="faulttolerant-input"
                ...
                <div class="tr" v-bind:style="!showAdvancedParameters ? 'visibility: collapse' : ''">
                    <div class="td">
                        <label for="numthreads-input">Num Threads</label>
                    </div>
                    <div class="td align-left">
                        <input-validated
                            id="numthreads-input"
                            type="text"
                            :parent-id="jobConfiguration.id"
                            :property-id="'numThreads'"
                            :validation-errors="validationErrors"
                            :model-value="jobConfiguration.numThreads"
        ...
        <div>
            <core-panel class="spacer-top">
                ...
                <div class="table margin-bottom">
                    <div class="tr">
                        ...
                        <div class="td">
                            <select
                                id="category"
                                :value="jobConfiguration.trigger.category.key"
                                @change="changeTriggerCategory($event)"
                                data-e2e="trigger-category-selector"
                            >
                                <option
                                    v-for="triggerCategory in triggerCategories"
                                    :value="triggerCategory.key"
                                    :key="triggerCategory.key"
                                >
                    ...
                    <div class="tr">
                        ...
                        <div class="td">
                            <select
                                id="type"
                                :value="jobConfiguration.trigger.type.key"
                                @change="changeTriggerParameters"
                ...
                <div class="info-box" v-if="jobConfiguration.trigger.type.supportsEvents">
                    <label
                        >This trigger fires on events. The job will stay in 'active' state and listen for incoming
                        events after start.<br /><br />
                        Not all actions are available for event-triggered jobs!<br /><br />
                        In order to simulate the job, you will have to provide incoming events to the trigger. If the
                        job is already active, incoming events might be processed by the simulated job execution!
                    </label>
                </div>
            </core-panel>
            ...
            <core-panel class="spacer-top">
                ...
                <p v-else>This trigger has no parameters to configure.</p>
            </core-panel>
        </div>
    </div>
</template>
...
<script>
import CorePanel from "../common/core-panel.vue";
import ParameterEditor from "../common/parameter-editor.vue";
import IgorBackend from "../../utils/igor-backend.js";
import IconButton from "../common/icon-button.vue";
import LayoutRow from "../common/layout-row.vue";
import InputValidated from "../common/input-validated.vue";
...
export default {
    ...
    methods: {
        loadTriggerCategories: async function () {
            await IgorBackend.getData("/api/category/trigger")
                .then((categories) => {
        ...
        loadTriggerTypesOfCategory: async function (categoryKey, selectFirst) {
            await IgorBackend.getData("/api/type/trigger/" + categoryKey)
                .then((types) => {
        ...
        loadTriggerParametersOfType: async function (typeKey) {
            ...
            await IgorBackend.getData("/api/parameters/trigger/" + typeKey)
                .then((parameters) => {
        ...
        arrowColor: function () {
            if (
                ...
                this.jobConfiguration.id in this.validationErrors &&
                ("historyLimit" in this.validationErrors[this.jobConfiguration.id] ||
                    "simulationLimit" in this.validationErrors[this.jobConfiguration.id])
            ) {
                return "arrow-alert";
            }
            return "";
